...
- (void)viewDidLoad {
    ...
    for (NSInteger i=0; i<IMAGECOUNT; i++) {
        ...
        if (isIp6) {
            imageName = [NSString stringWithFormat:@"iPhone6Plus-guide-%ld-v",i];
        }else{
            imageName = [NSString stringWithFormat:@"iPhone5-guide-%ld-v",i];
        }
...
-(void)setupEnterButton:(UIImageView *)imageView
{
    ...
    // 2.配置按钮
    //enterButton.frame = CGRectMake((imageView.frame.size.width-150)/2, imageView.frame.size.height*0.8, 150, 40);
    enterButton.width = 150;
    enterButton.height = 40;
    enterButton.x = (imageView.width-150)/2;
    enterButton.y = imageView.height*0.85;
...
-(void)enterApp
{
    [UIApplication sharedApplication].keyWindow.rootViewController = kVCFromSb(@"home",@"Main");
}
